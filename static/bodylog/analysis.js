...
(function () {
    ...
    const chartPoints = parseJsonScript("analysis-chart-points") ?? [];
    const targetWeightInputNode = byId("target-weight-input");
    const metricModeButtons = Array.from(document.querySelectorAll("[data-metric-mode]"));
    const chartTooltip = byId("chart-tooltip");
    const calendarNoteTooltip = byId("calendar-note-tooltip");
    ...
    function bindCalendarNoteTooltip() {
    ...
    function buildScaledPath(points, metric, bounds, dims) {
    ...
    function renderCombinedChart(targetValue, metricMode) {
        const svg = byId("combined-chart");
        if (!svg) {
            return;
        }
        const dims = { width: 1280, height: 420, left: 72, right: 72, top: 24, bottom: 48 };
